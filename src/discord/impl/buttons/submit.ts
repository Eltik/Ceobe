...
export default {
    ...
    execute: async (interaction: Interaction) => {
        if (interaction.isButton()) {
            ...
            const modal = 
                new ModalBuilder()
                    .setCustomId(`submit-${id}`)
                    .setTitle("Submit Challenge");
            
            const title = 
                new TextInputBuilder()
                    .setCustomId(`submit-${id}-title`)
                    .setLabel("Submission Title")
                    .setStyle(TextInputStyle.Short)
                    .setPlaceholder("Short title of your clear...");
            const description =
                new TextInputBuilder()
                    .setCustomId(`submit-${id}-description`)
                    .setLabel("Submission Description")
                    .setStyle(TextInputStyle.Paragraph)
                    .setPlaceholder("Short description of the submission...");
            const imageURL =
                new TextInputBuilder()
                    .setCustomId(`submit-${id}-image`)
